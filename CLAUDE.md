...
**목적**: 1분 이상 소요되거나 토큰을 많이 소모한 작업을 분석하고, 캐싱/최적화 방안을 기록하는 곳.
작업 일지나 구현 요약을 쓰는 곳이 아니다.

작업 중 다음 상황이 발생하면 feedback에 기록한다:
- 대용량 파일 반복 로딩이 병목으로 확인된 경우
- 알고리즘 복잡도 문제(O(n²) 등)가 발견된 경우
- 바이너리 캐싱이나 자료구조 교체로 개선 가능한 경우

```
feedback/
└── heavy_io_analysis.md    # 고비용 I/O 및 알고리즘 분석, 최적화 방안
```
